<?php
/**
 *  handbook custom functions.
 *
 *  @package handbook
 */

if ( ! function_exists( 'handbook_entry_footer' ) ) {
	/**
	 *  Show categories, tags, comment and edit links after post.
	 */
	function handbook_entry_footer() {
		if ( 'post' === get_post_type() ) {
			$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma', 'handbook' ) );
			if ( $categories_list ) : ?>
				<p class="cat"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'handbook' ) ); ?></p>
			<?php	endif;

			$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma', 'handbook' ) );
			if ( $tags_list ) {
...
if ( ! function_exists( 'handbook_comments' ) ) {
	/**
	 *  Custom comments function.
	 */
	function handbook_comments( $comment, $args, $depth ) {
...
function handbook_get_git_commit_info( $post_id = null ) {
	...
	$git_file_path = get_post_meta( $post_id, '_wpghs_github_path', true );
	$sha = get_post_meta( $post_id, '_sha', true );
	$last_commit_update_sha = get_post_meta( $post_id, '_last_commit_update_sha', true );
	$last_commit_timestamp = get_post_meta( $post_id, '_handbook_last_commit_timestamp', true );

	if ( $sha === $last_commit_update_sha ) {
